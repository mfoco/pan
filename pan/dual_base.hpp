...
namespace pan
{
    template <typename T, int S> class base
    {
        static_assert(S == 0 || S == -1 || S == 1, "Signature must be -1, 0 or 1.");
        constexpr T signature = S;
        T _value;
    public:
        explicit constexpr base(T value) noexcept : _value(value) {}

        constexpr T imag() const noexcept { return _b; }
        ...
        constexpr base<T> operator - () const noexcept(noexcept(-_value))
        {
            return{ -_value };
        }

        constexpr base<T> conj() const noexcept(noexcept(-_value))
        {
            return{ -_value };
        }

        /*constexpr*/ base<T> &operator += (const base<T> &other) noexcept(noexcept(_value += other._value))
        {
            _value += other._value;
            return *this;
        }
        ...
        /*constexpr*/ base<T> &operator *= (T other) noexcept(noexcept(_value *= other))
        {
            _value *= other._value;
            return *this;
        }
    ...
    template <typename T> using real = base<T, 1>;
    template <typename T> using imaginary = base<T, -1>;
    template <typename T> using epsilon = base<T, 0>;

    template <typename T> constexpr base<T> operator + (const base<T> &a, T b) noexcept(noexcept(a.real() + b))
    {
        return{ a.real() + b, a.dual() };
    }

    template <typename T> constexpr base<T> operator + (T a, const base<T> &b) noexcept(noexcept(a.real() + b))
    {
        return{ a + b.real(), b.dual() };
    }

    template <typename T> constexpr base<T> operator + (const base<T> &a, const base<T> &b) noexcept(noexcept(a.real() + b.real()) && noexcept(a.dual() + b.dual()))
    {
    ...
    template <typename T> constexpr base<T> operator - (T a, const base<T> &b) noexcept(noexcept(a - b.real()) && noexcept(-b.dual()))
    {
        return{ a - b.real(), -b.dual() };
    }
    ...
    template <typename T> constexpr base<T> operator * (const base<T> &a, T b) noexcept(noexcept(a.real() * b) && noexcept(a.dual() * b))
    {
        return{ a.real() * b, a.dual() * b };
    }

    template <typename T> constexpr base<T> operator * (T a, const base<T> &b) noexcept(noexcept(a * b.real()) && noexcept(a * b.dual()))
    {
        return{ a * b.real(), a * b.dual() };
    }

    template <typename T> base<T> operator * (const base<T> &a, const base<T> &b)  noexcept(noexcept(a.real() * b.real()) && noexcept(a.real()*b.dual() + a.dual()*b.real()))
    {
        return{ a.real()*b.real(), a.real()*b.dual() + a.dual()*b.real() };
    }

    template <typename T> base<T> operator / (const base<T> &a, T b) noexcept(noexcept(a.real() / b) && noexcept(a.dual() / b))
    {
        return{ a.real() / b, a.dual() / b };
    }

    template <typename T> base<T> operator / (T a, const base<T> &b) noexcept(noexcept(b.real() / a) && noexcept(a.dual() / b))
    {
        auto r = real(b);
        return (a * conj(b)) / (r*r);
    }

    template <typename T> auto operator / (const base<T> &a, const base<T> &b) noexcept(noexcept(a.real() / b.real()))
    {
        auto r = real(b);
        return (a * conj(b)) / (r*r);
    }

    template <typename T> decltype(auto) real(const base<T> &other) noexcept
    {
        return other.real();
    }

    template <typename T> decltype(auto) dual(const base<T> &other) noexcept
    {
        return other.dual();
    }

    template <typename T> auto abs(const base<T> &other)
    {
        using namespace std;
    ...
    template <typename T> base<T> conj(const base<T> &other)
    {
        return{ other.real(), -other.dual() };
    }

    template <typename T> auto norm(const base<T> &other)
    {
        decltype(auto) r = other.real();
        return r*r;
    }

    template <typename T> base<T> exp(const base<T> &other)
    {
        using std::exp;
    ...
    template <typename T> base<T> log(const base<T> &other)
    {
        using std::log;
        decltype(auto) r = other.real();
    ...
    template <typename T> base<T> pow(const base<T> &left, const base<T> &right)
    {
    ...
    template <typename T, typename T2> base<T> pow(const base<T> &left, const T2 &right)
    {
        return exp(right*log(left));
        //using std::pow;
        //decltype(auto) lr = left.real();
    ...
    template <typename T, typename T2> base<T> pow(const T2 &left, const base<T> &right)
    {
        using std::log;
        return exp(right*log(left));
        //using std::log;
    ...
    template <typename T> base<T> sin(const base<T> &other)
    {
        using namespace std;
        return{ ::sin(other.real()), ::cos(other.real())*other.dual() };
    }

    template <typename T> base<T> cos(const base<T> &other)
    {
        using namespace std;
        return{ ::cos(other.real()), -::sin(other.real())*other.dual() };
    }

    template<typename _Ty, typename _Elem, typename _Tr> inline std::basic_ostream<_Elem, _Tr>& operator<<(std::basic_ostream<_Elem, _Tr>& ostr, const base<_Ty>& right) {
